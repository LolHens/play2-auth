...
trait AuthActionBuilders extends AsyncAuth { self: AuthConfig with BaseController =>
  ...
  final case class GenericOptionalAuthFunction[R[+_] <: Request[_]]() extends ActionFunction[R, ({type L[+A] = GenericOptionalAuthRequest[A, R]})#L] {
    def invokeBlock[A](request: R[A], block: GenericOptionalAuthRequest[A, R] => Future[Result]): Future[Result] = {
      implicit val ctx: ExecutionContext = executionContext
      restoreUser(request: RequestHeader, ctx) recover {
  ...
  final def composeAuthenticationAction[R[+_] <: Request[_], B](builder: ActionBuilder[R, B]): ActionBuilder[({type L[+A] = GenericAuthRequest[A, R]})#L, B] = {
    composeOptionalAuthAction[R, B](builder).andThen[({type L[+A] = GenericAuthRequest[A, R]})#L](GenericAuthenticationRefiner[R]())
  }
